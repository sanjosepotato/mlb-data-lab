...
def main():
    ...
    JP_PITCHER_IDS = {
        808967: {"name": "Yoshinobu Yamamoto", "team": "LAD", "league": "NL"},
        681911: {"name": "Shota Imanaga",      "team": "CHC", "league": "NL"},
        660271: {"name": "Shohei Ohtani",      "team": "LAD", "league": "NL"},
        808963: {"name": "Roki Sasaki",        "team": "LAD", "league": "NL"},
    ...
    # 日本人投手を追加（IP基準を満たせば正規ランキングへ、未満はout_of_rankへ）
    # out_of_rank_pitchers: ランク外表示用（HTMLのOUT_OF_RANKで参照）
    out["outOfRankPitchers"] = {}
    cy_existing = {p["id"] for v in out["cyYoung"].values() for p in v}
    for pid, info in JP_PITCHER_IDS.items():
        ...
        entry = build_pitcher_entry(pid, s_fake, is_jp=True, compute_qs=(ip_raw >= MIN_IP))
        if ip_raw >= MIN_IP:
            # IP基準を満たす → 正規ランキングに追加
            out["cyYoung"][info["league"]].append(entry)
            print(f"    JP投手追加(正規): {info['name']} ERA={entry['era']} IP={entry['ip']} QS={entry['qs']}%")
        else:
            # IP基準未満 → ランク外データとして別保存（HTMLで参照）
            out["outOfRankPitchers"][str(pid)] = entry
            print(f"    JP投手追加(ランク外): {info['name']} ERA={entry['era']} IP={ip_raw} < MIN_IP={MIN_IP}")
    ...
    JP_IDS = {
        ...
        808967: {"nameJa": "山本 由伸",   "team": "LAD", "type": "pitcher"},
        808963: {"nameJa": "佐々木 朗希", "team": "LAD", "type": "pitcher"},
